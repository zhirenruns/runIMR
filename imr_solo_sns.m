...
Lmax = 10;                 % Max. stretch of bubble (=Rmax/Req)
tspan = 0.2*Rmax;               % Temporal duration studied (s)
...
mu_visc = 0.1;              % Viscous modulus (Pa*s)
tau1 = 1E-11;               % Relaxation time scale (s)
...
Sm0 = 0;                                            % Initial stress in Maxwell material. [To be calculated with a subroutine]
SI0 = -(5 - Lmax^(-4) - 4/Lmax)/(2*Ca) + Sm0;       % Stress integral

% Track internal variable in spatial node
load('GQMesh.mat',"GQMesh");
ngq = size(GQMesh,1);   % # of nodes in spatial mesh
LE0 = ones(1,ngq);      % Elastic distortion (Assume 1 for now, replace later)

params.GQMesh = GQMesh; % Pass with parameters

X0 = [Rb0,Vb0,pb0,SI0,Theta0,kv0,LE0]';
...
ngq = size(GQMesh,1);   % # of nodes in spatial mesh
indx_gq = indx_k + NY;
LE = X((indx_gq+1):(indx_gq+ngq));      % Elastic distortion
...
% Treat Maxwell branch:
xi = GQMesh(:,1);
wt = GQMesh(:,2);

Lbdot = Vb*Lmax;                    % stretch rate of bubble wall

Lxi = (Lb^3 + xi.^3 - 1)./xi;       % radial stretch at node
dLL =  (Lb^2)*(Lbdot)./(xi.*Lxi);   % ratio of stretch rate and stretch at node
LEdot = LE.*(dLL - log(LE)/De);     % rate of elastic distortion
sig_M = -4*log(LE)/(Re*De);         % radial stress at node 
sigdot_M = -4*(LEdot./LE)/(Re*De);  % radial stress rate at node

chunk1 = (xi.^2)./(Lb^3 + xi.^3 - 1);
chunk2 = (xi.^2)*(Lb^2)*(Lbdot)./(Lb^3 + xi.^3 - 1).^2;

S_M = sum(wt.*(3*sig_M.*chunk1));
Sdot_M = sum(wt.*(3*sigdot_M.*chunk1 - 9*sig_M.*chunk2));

% Combine:
SI = S_Ca + S_M;
SIdot = Sdot_Ca + Sdot_M;
...
dxdt = [Rdot; Vdot; pbdot; SIdot; Theta_prime; kv_prime; LEdot];
