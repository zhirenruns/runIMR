...
%
%   Special version for SLS: neo-Hookean + linear Maxwell
%           Linear Maxwell is a bad model. Using for pIMR paper only.
%
% =========================================================================
% References:
...
% (A) LIC Experiment 
Rmax = 0.000340865161032672;             % Max. radius of bubble (m)
Lmax = 6.91246313690901;                 % Max. stretch of bubble (=Rmax/Req)
tspan = Rmax;               % Temporal duration studied (s)
...
G_elast = 10081.0911;           % Elastic modulus (Pa)
mu_visc = 0.095866;             % Viscous modulus (Pa*s)
tau1 = 1.8958e-06;              % Relaxation time of Maxwell (s)
...
disp('Growth')
tic
S_SLS = get_S0_SLS(Lmax,Re,De,Ca);                   % Call external function to get S0 for Maxwell:
toc 

SI0 = -(5 - Lmax^(-4) - 4/Lmax)/(2*Ca) + S_SLS;     % Stress integral
...
Sdot_Maxwell = -(S_Maxwell + (4/Re)*Vb/Rb)/De;
SIdot = Sdot_Ca + Sdot_Maxwell;
